...
TeacherUI() {
    ...
    cids=($($mysql_prefix -se "$query_id;" 2>/dev/null))
    ...
    $mysql_prefix -e "$query_course;" 2>/dev/null
    while :; do
    ...
    echo "您选择的课程为："
    $mysql_prefix -e "select id 课程号, name_zh 中文名称, name_en 英文名称 from course where id=$cid;" 2>/dev/null

    tids=($($mysql_prefix -e "$query_tid and tid <> $tid;" 2>/dev/null))
    if [ ${#tids[@]} -gt 0 ]; then
        echo "与您一同教这门课的老师有："
        $mysql_prefix -e "$query_teacher and id <> $tid;" 2>/dev/null
    fi

    sids=($($mysql_prefix -e "$query_sid;" 2>/dev/null))
    if [ ${#sids[@]} -gt 0 ]; then
        echo "选上这门课的同学们有："
        $mysql_prefix -e "$query_student;" 2>/dev/null
    fi
...
TeacherManageCourse() {
    ...
    while :; do
        ...
        1)
            ...
            iids=($($mysql_prefix -e "$query_iid;" 2>/dev/null))
            if [ ${#iids[@]} -gt 0 ]; then
                echo "本课程已有的课程公告如下图所示"
                $mysql_prefix -e "$query_info;" 2>/dev/null
            fi
        ...
        2)
            ...
            $mysql_prefix -e "$query_brief_update;" 2>/dev/null
            break
            ;;
        *)
            echo "您输入的操作$op有误，请输入上面列出的操作"
